...
std::unique_ptr<Mesh> Model::processMesh(aiMesh *mesh, const aiScene *scene)
{
    ...
    // 1. diffuse maps
    std::vector<std::unique_ptr<Texture2D>> diffuseMaps = std::move(loadMaterialTextures(material, aiTextureType_DIFFUSE, "texture_diffuse"));
    for(std::unique_ptr<Texture2D>& texture : diffuseMaps)
        textures.push_back(std::move(texture));
    // 2. specular maps
    std::vector<std::unique_ptr<Texture2D>> specularMaps = std::move(loadMaterialTextures(material, aiTextureType_SPECULAR, "texture_specular"));
    for(std::unique_ptr<Texture2D>& texture : specularMaps)
        textures.push_back(std::move(texture));
    // 3. normal maps
    std::vector<std::unique_ptr<Texture2D>> normalMaps = std::move(loadMaterialTextures(material, aiTextureType_HEIGHT, "texture_normal"));
    for(std::unique_ptr<Texture2D>& texture : normalMaps)
        textures.push_back(std::move(texture));
    // 4. height maps
    std::vector<std::unique_ptr<Texture2D>> heightMaps = std::move(loadMaterialTextures(material, aiTextureType_AMBIENT, "texture_height"));
    for(std::unique_ptr<Texture2D>& texture : heightMaps)
        textures.push_back(std::move(texture));
...
std::vector<std::unique_ptr<Texture2D>> Model::loadMaterialTextures(aiMaterial *mat, aiTextureType type, std::string typeName)
{
    std::vector<std::unique_ptr<Texture2D>> textures;
    for(unsigned int i = 0; i < mat->GetTextureCount(type); i++)
    {
        ...
        {   // if texture hasn't been loaded already, load it
            std::unique_ptr<Texture2D> texture = std::make_unique<Texture2D>(this->directory + "/" + str.C_Str());
